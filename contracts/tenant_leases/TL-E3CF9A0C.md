# TOWER SITE SUBLICENSE AGREEMENT — T-MOBILE

**Sublicense ID:** TL-E3CF9A0C  
**Tower Site ID:** CC-TWR-480303  
**MLA Reference:** MLA-T-MO-2021

---

**THIS TOWER SITE SUBLICENSE AGREEMENT** ("Sublicense Agreement") is entered into as of April 01, 2009 ("Commencement Date"), by and between:

**LICENSOR:**  
Summit USA Inc.  
2000 Corporate Drive  
Canonsburg, PA 15317  
("Summit" or "Licensor")

**LICENSEE:**  
T-Mobile USA Inc.  
12920 SE 38th St., Bellevue, WA 98006  
("T-Mobile" or "Licensee")

---

## RECITALS

WHEREAS, Licensor operates tower **CC-TWR-480303** in Rockford, IL;

WHEREAS, Licensee desires to install equipment pursuant to **MLA-T-MO-2021**;

NOW, THEREFORE, the parties agree:

---

## SECTION 1: LICENSED SPACE

**1.1 Tower Space.** Licensee receives up to **8 antenna(s)** at **69 ft** RAD center, and **2 RRU(s)**.

**1.2 Equipment Manifest:**

| Item | Qty | Model | Location |
|------|-----|-------|----------|
| Panel Antenna | 8 | CommScope SBNHH-1D65C | 69ft, 2 sector(s) |
| RRU | 2 | Nokia AirScale AAHF B41/B71 | 69ft |
| Fiber | 1 | 12-strand SM OS2 | Tower to cabinet |
| Coax | 8 | 7/8" feedline | Tower to ground |

---

## SECTION 2: TERM

**2.1 Term.** 5 years, from April 01, 2009 to 31 March 2014.

**2.2 Zone:** Suburban | **Status:** Active

---

## SECTION 3: MONTHLY RENTS

**3.1 Monthly Rent.** **$4,126.47/month** ($49,517.64/year).

**3.2 Escalation.** 3.0% annually.

**3.3 Breakdown:**
- Antenna space: $2,269.56/mo
- Ground space: $1,031.62/mo
- Power/utilities: $825.29/mo

---

## SECTION 4: INSTALLATION & MAINTENANCE

**4.1 Standards.** Per TIA-222-H, FCC limits, and Licensor specs.
**4.2 Access.** 24/7 for maintenance and emergencies.
**4.3 Inspections.** Quarterly visual; annual RF verification.

---

## SECTION 5: INSURANCE

CGL: $5,000,000 per occurrence. Licensor as additional insured. Licensee indemnifies Licensor from all claims.

---

## SECTION 6: INTERFERENCE

Licensee shall not cause interference to prior installations. Priority based on installation date.

---

## SECTION 7: TERMINATION

Licensee may terminate on 180 days' notice. Equipment removal within 180 days of termination. Nonpayment after 30 days' notice = default.


**Sprint Merger Note.** This license originated with Sprint Corp. Rights assumed by T-Mobile per FCC merger approval (April 2020). Equipment consolidation applies.

---

**LICENSOR:** Summit USA Inc.  
By: \_\_\_\_ / Date: \_\_\_\_

**LICENSEE:** T-MOBILE USA INC.  
By: \_\_\_\_ / Date: \_\_\_\_

---
*Exhibits: Site Plan (CC-TWR-480303), Equipment Layout, MLA (MLA-T-MO-2021)*